...
- analyze 이후 해석은 `AnalyzeResult`를 입력으로 받아 안정된 scenario snapshot을 산출하는 별도 단계로 다룬다. 현재 matrix 구현은 raw 축 점수를 정책별 signal label로 분류한 뒤 scenario를 선택하며, 이 label snapshot은 해석 결과 계약에 포함할 수 있다.
...
- 현재 응답 계약은 `scenario, summary, signals`를 기준으로 유지한다.
- `scenario`는 안정 식별자이며, `summary`는 교체 가능한 렌더러가 생성하는 표현값으로 본다.
- `signals`는 `trend, volatility, location, pressure` 4축의 label snapshot이며, 현재 scenario 선택에 실제 사용된 해석 근거를 전달하는 계약으로 본다.
...
- raw analyze 축 점수(`trend`, `volatility`, `location`, `pressure_*`)는 현재 외부 응답 계약에 직접 노출하지 않는다. 외부에는 raw 값 대신 4축 signal label snapshot만 노출한다.
